...
public:
    ...
    // Public accessors for the pressure field (used by solvers)
    std::vector<std::vector<std::vector<double>>>& pressureField() { return p_; }
    const std::vector<std::vector<std::vector<double>>>& pressureField() const { return p_; }
